...
- `app/src/main/java/com/mapshoppinglist/data/dao/PlaceDao.kt`（新規作成の可能性）

**実装内容:**
- 地点ごとのアイテム取得クエリを追加
- アイテム数付き地点リスト取得クエリを追加
- データクラス定義（PlaceWithItems など）

**テスト:**
- 単体テスト作成（クエリの動作確認）

### 2.3 ⬜ 未着手: Repositoryレイヤーの拡張
**対象ファイル:**
...
**実装内容:**
- 地点グループデータ提供メソッドの追加
- Flow による リアクティブなデータ提供

**テスト:**
- Repository の単体テスト作成

### 2.4 ⬜ 未着手: ViewModelの拡張
**対象ファイル:**
- `app/src/main/java/com/mapshoppinglist/ui/home/ShoppingListViewModel.kt`

**実装内容:**
- タブ選択状態の管理（StateFlow）
- 地点グループデータの提供（StateFlow）
- タブ切り替えロジック

**テスト:**
- ViewModel の単体テスト作成

### 2.5 ⬜ 未着手: UIコンポーネントの実装
**対象ファイル:**
